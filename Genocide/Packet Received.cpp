...
DWORD FASTCALL OnGamePacketReceived(LPBYTE Packet, DWORD Length)
{
	...
	case 0x0D:
	{
		...
		SetBlind(*(LPWORD)& Packet[7], *(LPWORD)& Packet[9], NULL, NULL, *(LPDWORD)& Packet[2]);
	}
	break;
	case 0x0E:
	...
	case 0x4c:
	{
		...
		if (*(LPDWORD)& Packet[6] == D2S_TELEPORT)
			if (*(LPDWORD)& Packet[10] == Me->dwUnitId)
				SetBlind(Me->pPath->xPos, Me->pPath->yPos, Unit->pPath->xPos, Unit->pPath->xPos, Unit->dwUnitId);
	...
	case 0x4d:
	{
		...
		if (*(LPDWORD)& Packet[6] == D2S_TELEPORT || *(LPDWORD)& Packet[6] == D2S_CHARGE || *(LPDWORD)& Packet[6] == D2S_WHIRLWIND)
			SetBlind(*(LPWORD)& Packet[11], *(LPWORD)& Packet[13], *(LPWORD)& Packet[11], *(LPWORD)& Packet[13], Unit->dwUnitId);
		else
			SetBlind(Unit->pPath->xPos, Unit->pPath->yPos, *(LPWORD)& Packet[11], *(LPWORD)& Packet[13], Unit->dwUnitId);
	}
	break;
	...
	case 0x8C:
		...
		{
			if (*(LPDWORD)& Packet[1] == Me->dwUnitId && !IsTown(Me))
			{
				if (Portal())
				{
