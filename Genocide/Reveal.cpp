...
VOID Reveal::RevealAutomap()
{
	...
	if (!AutoRevealAutomap && m_RevealedActs[dwAct])
		return;

	for (INT nLevel = m_ActLevels[dwAct]; nLevel < m_ActLevels[dwAct + 1]; nLevel++)
	{
		LPLEVEL pLevel = GetLevels(pAct->pMisc, nLevel);
		...
		if (pUnit->dwType == UNIT_TYPE_TILE)
		{
			...
			if (dwTargetLevel)
			{
				...
				if (dwTargetLevel == pRoom->pLevel->pMisc->dwStaffTombLevel)
					strcat_s(LevelDesc->szName, StaffTombIndicator);
...
{
	LPMYAUTOMAPLAYERMAN pMyLayerMan = &MyAutoMapLayer[AutoMapLayer->nLayerNo];
...
{
	for (INT i = 0; i < ArraySize(MyAutoMapLayer); i++)
	{
		LPMYAUTOMAPLAYER pMyLayer = MyAutoMapLayer[i].pMyLayer;
	...
	::memset(MyAutoMapLayer, 0, sizeof(MyAutoMapLayer));
}
...
{
	LPMYAUTOMAPLAYERMAN MyLayerMan = &MyAutoMapLayer[AutoMapLayer->nLayerNo];
...
LPLEVEL Reveal::GetLevels(LPACTMISC pMisc, DWORD dwLevelNo)
{
...
LPAUTOMAPLAYER2 Reveal::InitAutomapLayer(DWORD levelno)
{
	...
	return (LPAUTOMAPLAYER2)Stubs::D2CLIENT_InitAutomapLayer_STUB(pLayer->nLayerNo);
}
...
VOID STDCALL AutomapCells(LPCELLCONTEXT Context, DWORD X, DWORD Y, RECT * Clip, DWORD Bright)
{
	...
	if (ShrineImages && Cell == CELLNO_WAYPOINT && WaypointCell)
	{
		Context->nCellNo = 0;
		Context->pCellFile = WaypointCell;
		X += (D2CLIENT_GetAutomapSize() ? 4 : 8) - (WaypointCell->cells[0]->width / 2);
		Y += (D2CLIENT_GetAutomapSize() ? 4 : 0);
	}

	if (((Cell -= CELLNO_MYSHRINES) >= 0) && (Cell < TOTALSHRINES))
	{
		LPCELLFILE MyShrine = MyShrineCells[D2CLIENT_GetAutomapSize() & MiniShrine][Cell];

		if (ShrineImages && MyShrine)
		{
...
if (Item->pItemData->dwFlags & 0x400000 && Ethereal)
{
CHAR Ethereal[5] = "[E]";
